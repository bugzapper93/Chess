    ...
    class AIPlayer
    {
        ...
        public Move GetBestMove(Chessboard board)
        {
            if (playerColor == Pieces.Black)
            {
                ...
                var allPossibleMoves = Moves.GetAllMoves(board, playerColor);

                var possibleMoves = new Moveset
                {
                    moves = allPossibleMoves.moves
                        .Where(move => (board.pieces[move.startPosition.row, move.startPosition.column].value & 24) == playerColor)
                        .ToList(),
        ...
        private int Minimax(Chessboard board, int depth, int alpha, int beta, bool isMaximizing)
        {
            ...
            var moves = OrderMoves(Moves.GetAllMoves(board, playerColor), board);
        ...
        private static int EvaluateBoard(Chessboard board, int playerColor)
        {
            ...
            foreach (var move in Moves.GetAllMoves(board, playerColor).moves)
            {
                if (move.capture)
                {
                    int attackPiece = board.pieces[move.startPosition.row, move.startPosition.column].value;
                    int capturedPiece = board.pieces[move.targetPosition.row, move.targetPosition.column].value;

                    int victimValue = Pieces.GetPieceValue(capturedPiece);
                    if ((attackPiece & 24) == Pieces.White)
        ...
        private int GetMoveScore(Move move, Chessboard board)
        {
            ...
            if (move.capture)
            {
                ...
                if ((attackPiece & 7) == Pieces.Pawn && move.capture && capturedPiece == 0)
                    capturedPiece = Pieces.Pawn;

                int victimValue = Pieces.GetPieceValue(capturedPiece);
                int attackerValue = Pieces.GetPieceValue(attackPiece);
